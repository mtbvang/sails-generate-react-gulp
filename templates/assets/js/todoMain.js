...
require([
		'react',
		'app/TodoModel',
		'app/TodoApp' ], function(React, TodoModel, TodoApp) {
	...
	function render() {
		...
		// );
		React.render(React.createFactory(TodoApp)({
			model : model
		}), document.getElementById('todoapp'));
	}
	...
	socket.on('todo', function whenMessageRecevied(message) {
		...
		// TODO brute force it. should really check the type of message and respond
		// accordingly.
		// for now we just update the whole list (this requires the mirror option in
		// config/blueprint)
